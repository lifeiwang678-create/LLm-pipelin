...
class HHARLoader:
    ...
    def load(self, subjects: Iterable[str] | None, labels: list[int]) -> list[SensorSample]:
        ...
        if not csv_paths:
            print(f"[skip] No HHAR CSV files found in {self.data_dir}")
            return []
    ...
    def _load_csv(self, csv_path: Path, subject_filter: set[str], labels_to_keep: set[int]) -> list[SensorSample]:
        ...
        subject_col = _first_existing(df, ["subject", "user", "user_id", "device", "phone"])
        label_col = _first_existing(df, ["label", "gt", "activity", "class"])
        if label_col is None:
            raise ValueError(f"HHAR file {csv_path} does not contain a label/activity column.")

        time_col = _first_existing(df, ["timestamp", "time", "Creation_Time", "Arrival_Time"])
        if time_col:
