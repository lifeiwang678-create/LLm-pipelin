<#
Run 48 MAIN/full-subset combinations locally on Windows PowerShell with Gemini 3.5 Flash.
...
Usage:
  ...
  powershell -ExecutionPolicy Bypass -File .\run_3datasets_72_local-Gemini.ps1
...
Notes:
  ...
  - This script uses --lm-provider gemini and model gemini-3.5-flash.
...
$LOGROOT = $env:LOGROOT
if (-not $LOGROOT) {
    $LOGROOT = Join-Path $BASE ('logs\llm_48_gemini35flash_noagent_main_' + (Get-Date -Format "yyyyMMddHHmmss"))
}
...
Write-Host "LM provider: $LM_PROVIDER"
