<#
Run 48 MAIN/full-subset combinations locally on Windows PowerShell with DeepSeek V4 Pro.
This version does NOT run multi_agent.
...
Usage:
  ...
  powershell -ExecutionPolicy Bypass -File .\run_3datasets_72_local-deepseek-v4-pro.ps1
...
Notes:
  ...
  - This script uses the OpenAI-compatible DeepSeek endpoint.
...
$LOGROOT = $env:LOGROOT
if (-not $LOGROOT) {
    $LOGROOT = Join-Path $BASE ('logs\llm_48_deepseak_deepseekv4pro_noagent_main_fewshot3_' + (Get-Date -Format "yyyyMMddHHmmss"))
}
...
if (-not $API_KEY) {
    Write-Error "ERROR: DEEPSEEK_API_KEY is not set. Set DEEPSEEK_API_KEY before running this script."
    exit 1
}
...
Write-Host "LM usages: direct, few_shot"
Write-Host "multi_agent: disabled"
...
$statusCsv = Join-Path $LOGROOT "deepseak_status.csv"
...
function Invoke-One {
    ...
    $logFile = Join-Path $LOGROOT "deepseak_${dataset}_${inputType}_${lm}_${output}.log"
