...
Environment:
  ...
  Data:      D:\DATA\experiment_4x3x2\Processed
  Provider:  OpenAI-compatible GPT API
...
foreach ($dataset in $datasets) {
    foreach ($inputType in $inputTypes) {
        ...
        if (-not (Test-Path $subsetCache)) {
            ...
            Write-Error "Run prepare_data_subsets.py and prepare_subset_inputs.py before this script."
            exit 1
        }
    }
}
    ...
    foreach ($inputType in $inputTypes) {
        foreach ($lm in $lms) {
            foreach ($output in $outputs) {
                Run-One -dataset $dataset -inputType $inputType -lm $lm -output $output
            }
        }
    }
}
